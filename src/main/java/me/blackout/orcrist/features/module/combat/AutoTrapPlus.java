...
public class AutoTrapPlus extends Module {
    ...
    private final Setting<Placement> placing = sgGeneral.add(new EnumSetting.Builder<Placement>().name("placing").description("How blocks are placed.").defaultValue(Placement.Instant).build());
    private final Setting<List<Block>> blocks = sgGeneral.add(new BlockListSetting.Builder().name("whitelist").description("Which blocks to use.").defaultValue(Blocks.OBSIDIAN, Blocks.NETHERITE_BLOCK).build());
    ...
    private final Setting<Integer> delay = sgGeneral.add(new IntSetting.Builder().name("place-delay").description("How many ticks between block placements.").defaultValue(1).visible(() -> placing.get() == Placement.Ticked).build());
    ...
    private final Setting<SettingColor> nextSideColor = sgRender.add(new ColorSetting.Builder().name("next-side-color").description("The side color of the next block to be placed.").defaultValue(new SettingColor(227, 196, 245, 10)).build());
    private final Setting<SettingColor> nextLineColor = sgRender.add(new ColorSetting.Builder().name("next-line-color").description("The line color of the next block to be placed.").defaultValue(new SettingColor(227, 196, 245)).build());
    ...
    public AutoTrapPlus() {
        super(OrcristAddon.Combat, "auto-trap+", "Traps people in a box to prevent them from moving.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        for (Block currentBlock : blocks.get()) {
            ...
            if (placing.get() == Placement.Instant && placePositions.size() > 0) {
            ...
            } else if (timer >= delay.get() && placePositions.size() > 0) {
                BlockPos blockPos = placePositions.get(placePositions.size() - 1);
