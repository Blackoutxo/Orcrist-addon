...
public class MainBotUtils {
    public static void toggle(Thread thread) {
        suspend(thread);
        Module m = Modules.get().get(ElytraBot.class);
        if (m.isActive()) m.toggle();
    }

    public static void suspend(Thread thread) {
        if (thread != null) thread.checkAccess();
    }
    ...
    public static boolean shouldEatItem(Item item) {
        if (item == Items.ENCHANTED_GOLDEN_APPLE || item == Items.GOLDEN_APPLE && !Modules.get().get(ElytraBot.class).allowGaps.get()) return false;
    ...
    public static void mine(BlockPos blockPos) {
        ...
        ElytraBot.sleepUntil(() -> !isSolid(blockPos), 15000);
    }
