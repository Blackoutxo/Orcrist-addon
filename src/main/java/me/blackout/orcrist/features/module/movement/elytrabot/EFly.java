...
public class EFly {
    ...
    public static void setMotion(BlockPos pos, BlockPos next, BlockPos previous) {
        ...
        double speed = Modules.get().get(ElytraBot.class).flySpeed.get();
        ...
        try {
            ...
            if (amount > 1) {
                speed = Modules.get().get(ElytraBot.class).maneuverSpeed.get();
                ...
                if (next.getX() - previous.getX() == next.getZ() - previous.getZ() && next.getY() - previous.getY() == 0) {
                    if (xDiff >= 1 && zDiff >= 1 || xDiff <= -1 && zDiff <= -1) {
                        speed = Modules.get().get(ElytraBot.class).flySpeed.get();
                    }
                }
            }
        ...
        if ((int) yDiff > 0) {
            y = Modules.get().get(ElytraBot.class).maneuverSpeed.get();
        } else if ((int) yDiff < 0) {
            y = -Modules.get().get(ElytraBot.class).maneuverSpeed.get();
        }
