...
import static me.blackout.orcrist.features.module.movement.elytrabot.botutil.MainBotUtils.getBlock;
import static me.blackout.orcrist.features.module.movement.elytrabot.botutil.MainBotUtils.isInRenderDistance;
import static me.blackout.orcrist.features.module.movement.elytrabot.botutil.MainBotUtils.isSolid;
        ...
        outer: for (BlockPos pos : positions2) {
            if (!isSolid(pos) && !closed.contains(pos)) {
                ...
                for (BlockPos check : checkPositions2) {
                    if (Modules.get().get(ElytraBot.class).botPathMode.get() == ElytraBot.PathMode.Highway && !isInRenderDistance(check)) {
                        return null;
                    }
                    ...
                    if (getBlock(check) == Blocks.LAVA && Modules.get().get(ElytraBot.class).avoidLava.get()) {
                        continue outer;
                    }

                    int maxY = Modules.get().get(ElytraBot.class).maxY.get();
